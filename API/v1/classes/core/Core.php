...
class CorefileAutoLoader {
	static function loadCores() {
		$files = scandir("./classes/core");
		foreach($files as $i=>$filename) {
			if ($filename != "Core.php" && preg_match("/^.+\.php$/", $filename)) {
				include "./classes/core/".$filename;
			}
		}
	}
}
...
class Core {
	...
	static function getBaseDirectoryName() {
		...
		*/
		return basename(str_replace(array("core", "classes"), "", dirname(__FILE__)));
	}
